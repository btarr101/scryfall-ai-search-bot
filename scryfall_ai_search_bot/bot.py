...
from scryfall_ai_search_bot.controller import Controller
...
class DiscordClient(discord.Client):
    ...
    async def on_message(self, message: discord.Message):
        ...
        async with message.channel.typing():
            ...
            scryfall_search_page_url = (
                f"https://scryfall.com/search?q={urllib.parse.quote(result.search_query)}"
            )

            view = View()
            view.add_item(Button(label="Open on Scryfall", url=scryfall_search_page_url, emoji="🔎"))

            formatted_reply = textwrap.dedent(
                f"""
                {result.explanation}\n
                ### Query:
                ```
            ...
            reply = await message.reply(
                formatted_reply, embed=discord.Embed(description="⏳ Querying Scryfall..."), view=view
            )

            scryfall_request = ScryfallAPIRequestBuilder().cards().search(result.search_query)
            try:
                first_page = await scryfall_request.aexecute(self.httpx_client) # TODO: Some kind of auto retry to correct itself and go back + edit
